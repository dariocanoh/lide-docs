...
========
  ...
  ``[lide.widgets] ^1.0`` ``lide.widgets.controls.combobox 1.0``


The Combobox control is used to display data in a drop-down combo box. 
By default, the Combobox control appears in two parts: the top part is 
a text box that allows the user to type a list item. The second part 
is a list box that displays a list of items from which the user can 
select one.
...
^^^^^^^^^^^^^^^^^
	...
	Finds and returns a reference to a ListItem object in a Combobox 
  control.
