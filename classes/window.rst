...
.. note::
  ...
  A Window is an object whose size and position can (usually) be 
...
^^^^^^^^^^^
  ...
  Shows the Window. Returns true if the window has been shown or 
  hidden or false if nothing was done because it already was in the 
  requested state.
...
^^^^^^^^^^^^^^^^^^^^^^
  ...
  Creates a status bar at the bottom of the frame. "nFields" are the 
  number of fields to create. Specify a value greater than 1 to create 
...
^^^^^^^^^^^^^^^^^^^^^^
  ...
  Sets the widths of the fields in the status bar. You must be send 
  many widths as fields exists.
