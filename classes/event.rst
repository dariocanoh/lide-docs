...
=====
  ...
  ``[lide.widgets] ^1.0`` ``lide.classes.event 1.0``


An event is a structure holding information about an event passed to 
a callback or member function. 
Event is used to be a multipurpose event object, and is an abstract 
...
^^^^^^^^^^^^^^^^^
   ...
   Returns the event handler function (that is called when you call 
   `Event:call`).
